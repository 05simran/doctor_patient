  ...
  useEffect(() => {
    const debounce = setTimeout(() => {
      if (patientData.problem) {
        getMedicineRecommendations(patientData.problem).then(setMedicineRecommendations)
      }
    }, 500)
      ...
      <form onSubmit={handleSubmit}>
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select name="gender" onValueChange={(value) => setPatientData((prev) => ({ ...prev, gender: value }))}>
              <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Textarea id="problem" name="problem" value={patientData.problem} onChange={handleInputChange} required />
          </div>

          <div className="space-y-2">
            <Label htmlFor="recommendedMedicine">Recommended Medicine</Label>
            <Select
              name="recommendedMedicine"
              onValueChange={(value) => setPatientData((prev) => ({ ...prev, recommendedMedicine: value }))}
            >
              <SelectTrigger>
              ...
              <SelectContent>
                {medicineRecommendations.map((medicine, index) => (
                  <SelectItem key={index} value={medicine}>
                    {medicine}
                  </SelectItem>
                ))}
              </SelectContent>
